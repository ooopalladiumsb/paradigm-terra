...
func makeValidator(cal canonical.Value, calHash string, snapshot canonical.Value, trace ExecutionTrace) (func() (*ValidationResult, error), func() (*ValidationResult, error), func() []canonical.Value) {
	...
	phaseA := func() (*ValidationResult, error) {
	...
	return phaseA, phaseB, func() []canonical.Value { return events }
}

// Validate is the atomic composition of ValidateToValidated + ResumeFromValidated (Gate #3):
// byte-identical to the pre-staging monolith.
func Validate(cal canonical.Value, calHash string, snapshot canonical.Value, trace ExecutionTrace) (*ValidationResult, error) {
	phaseA, phaseB, _ := makeValidator(cal, calHash, snapshot, trace)
	term, err := phaseA()
	if err != nil {
		return nil, err
	}
...
// ToValidated is the stage-1 result: Terminal set on a pre-validation failure (Events its
// failure event), else Events = [cal.validated] and the CAL is left in-flight at VALIDATED.
type ToValidated struct {
	Terminal *ValidationResult
	Events   []canonical.Value
}

// ValidateToValidated runs stage 1 (gates 1-7 -> cal.validated).
func ValidateToValidated(cal canonical.Value, calHash string, snapshot canonical.Value, trace ExecutionTrace) (*ToValidated, error) {
	phaseA, _, getEvents := makeValidator(cal, calHash, snapshot, trace)
	term, err := phaseA()
	if err != nil {
		return nil, err
	}
	if term != nil {
		return &ToValidated{Terminal: term, Events: term.Events}, nil
	}
	return &ToValidated{Terminal: nil, Events: getEvents()}, nil
}

// ResumeFromValidated runs stage 2 (gates 8-13) on a VALIDATED CAL; EXPIRED_POST when tick > expiration.
func ResumeFromValidated(cal canonical.Value, calHash string, snapshot canonical.Value, trace ExecutionTrace) (*ValidationResult, error) {
	_, phaseB, _ := makeValidator(cal, calHash, snapshot, trace)
	return phaseB()
}
