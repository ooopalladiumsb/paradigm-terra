...
byte-for-byte by Rust + Go (135 checks each), promoted PRE-NORMATIVE → NORMATIVE on
2026-05-25. The DSL portion of each cost reuses `expression_cost`/`ExpressionCost`,
newly exposed on the DSL ports, so it is parity-locked to the numbers the DSL layer
already pins.
