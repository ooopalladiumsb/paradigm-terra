...
| **`rust-parity` runner stabilization** ▶ **M1, in progress** | provision the CI runner for the repo's musl-static / `rust-lld` build so the optional `rust-parity` job goes green, then **promote it to required** | PATCH→policy | the second CI finding (`release-gate.md §CI findings`): RED is purely environmental, locally green across all 8 crates. Fix landed: `targets: x86_64-unknown-linux-musl` on the toolchain step (`post-release/m1-rust-ci-runner`); promotion to required pending one green observation. |
