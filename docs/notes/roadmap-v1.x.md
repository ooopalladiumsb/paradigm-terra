...
|---|---|---|---|
| **Registry deployment** ▶ **M2, ACTIVE (scoped)** | M2 = Registry **reconciliation** contract (settlement observation) for the proven `send_ton` path — see `m2-charter.md` | MINOR | scoped by architect ruling 2026-06-09: reconciliation role, `send_ton` only, offline-first (testnet leg gated), non-normative contract. Closes the §6.4 emission≠settlement gap (`cal-to-w5-mapping-review.md`). New verbs / jetton / nft / normative promotion are **Tier C, OUT**. Branch `post-release/m2-registry-reconciliation`. |
