...
| **Remote / incremental backups** ▶ **M3, ACTIVE** | extend PR-1.7 local backup→restore to off-host and incremental (snapshot + WAL deltas) — see `m3-charter.md` | MINOR | M3-A (incremental) + M3-C (remote sink, real cloud gated). Architect ruling 2026-06-10: offline-first, sink abstraction; the round-trip-to-identical-`STATE_ROOT` invariant is the acceptance bar. Branch `post-release/m3-durability`. |
| **WAL archival / compaction** ▶ **M3, ACTIVE** | bound on-disk growth: archive/compact the event log behind snapshots without breaking byte-exact replay — see `m3-charter.md` | MINOR | M3-B. Must preserve §7.2 replayability (replay-from-compacted == replay-from-full) — root values unchanged. |
