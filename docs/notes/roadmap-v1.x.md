...
| **Distributed observers** | scale PR-1.8's single live observer to multiple independent tailers (consensus on the published root) | MINOR | strengthens H3.5-live into a fleet; observe-only, decides nothing. (Remaining Tier-M, not yet scheduled.) |
| **Long-duration soak program** ▶ **A1, ACTIVE** | run the 7–30 day continuous soak the PR-1 charter named (PR-1.9 proved the gate over 120 ticks) — see `a1-soak-charter.md` | PATCH/process | architect ruling 2026-06-10: extends the PR-1.9 `SoakMonitor` to SC-1..6 (adds restore-equivalence + fd/disk sampling); program + accelerated CI proof in-repo, the 7/30-day wall-clock run is OPERATED. Zero violations + zero Freeze-Surface defect is the bar. Branch `post-release/a1-soak`. |
