...
- **MCP schema hash**: ожидаемый хеш `@ton/mcp` схемы фиксируется при деплое и проверяется при каждом запуске. Формат: `SHA256("PARADIGM_TERRA_MCP_V1" || canonical_json(mcp_schema))`. Значение публикуется в отдельном параметре конституции при переходе к Conformance Freeze.
